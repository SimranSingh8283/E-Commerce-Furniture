 ...
 */
const queueCallback = (cb, e, t = false) => {
    if (!t) {
...
const on = (type, el, listener, all = false) => {
    const elements = typeof el === "string" || el instanceof NodeList
        ? select(el, all)
        : el;
...
function getLuminance(color) {
    ...
    const [r, g, b] = match.slice(0, 3).map(Number).map(v => v / 255);
...
export { getScrollbarWidth, select, on, wait, setAttributes, findClosestTarget, queueCallback, debounce, _throttle, getLuminance, rgbToRgba, useMedia, reflow }